...
function startNodeExpressServer() {
    ...
    const data = [
        ...
        <body>
        <pre>Cannot GET /api/getDataByCategory/ui</pre>
        {this.var2}
        </body>
        </html>`,
    ...
    app.get('/api/data/:template', (req, res) => {
        ...
        if (templateMode === 'eval') {
            ...
            for (i = 1; i < 10; i++) {
                objectName = "var" + i;
                if (firstItem[objectName])
                    substituteEmbeddedTemplate(firstItem, objectName);
            }
            substituteEmbeddedContent(firstItem);
        }
        res.send(shallowData);
    });
    ...
    app.get('/api/getDataByTag/:tag', (req, res) => {
